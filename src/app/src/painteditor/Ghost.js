...
export default class Ghost {
  ...
  static findTarget(evt) {
    ...
    if (
      Ghost.outsideArea(
        Vector.floor(Vector.scale(pt, Paint.currentZoom)),
        maskCanvas
      )
    ) {
      return null;
  ...
  static findWho(evt) {
    ...
    return mt && mt.getAttribute("fixed") != "yes"
      ? mt
      : Ghost.getHitObject(pt);
  }
  ...
  static notHitted(pt) {
    ...
    switch (Paint.mode) {
      ...
      case "stamper":
      case "scissors":
        mt = Ghost.getActualHit(
          Ghost.getHitObject(pt, Paint.mode != "path"),
          pt
        );
        if (mt && mt.id) {
  ...
  static hittedSingleObject(obj, pt) {
    ...
    ctx.clearRect(
      0,
      0,
      ScratchJr.workingCanvas.width,
      ScratchJr.workingCanvas.height
    );
    ctx.save();
  ...
  static getHitObject(pt, isTip, exclude) {
    ...
    return Ghost.findHit(
      list,
      pt,
      ScratchJr.workingCanvas.getContext("2d"),
      isTip,
      exclude
    );
  }
