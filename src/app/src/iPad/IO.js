...
export default class IO {
        ...
        if (md5 && typeof md5 !== "undefined") {
          if (md5.indexOf("samples/") < 0) {
            ...
            if (collectLibraryAssets) {
              ...
              if (
                projectMetadata[assetType].indexOf(md5) < 0 &&
  ...
  static uniqueProjectName(jsonData, callback, useOne) {
    ...
    IO.query(iOS.database, json, function (existingProjects) {
      ...
      if (
        newNumber != null &&
        (!giftProjectNameParts.number ||
          newNumber > giftProjectNameParts.number)
      ) {
  ...
  static loadProjectFromSjr(b64data) {
    ...
    receivedZip.forEach(function (relativePath, file) {
      ...
      if (fullName == "data.json") {
        ...
        var projectVersion =
          parseInt(jsonData.version.replace("iOSv", "")) || 0;
    ...
    receivedZip.forEach(function (relativePath, file) {
      ...
      if (subFolder == "thumbnails" || subFolder == "sounds") {
        ...
        iOS.setmedianame(b2data, name, ext, function () {
          ...
          var width =
            svgParser.getElementsByTagName("svg")[0].width.baseVal.value;
          var height =
            svgParser.getElementsByTagName("svg")[0].height.baseVal.value;
          var scale = "0.5"; // fixed value - see PaintIO

          IO.getImagesInSVG(data, gotSVGImages);

          function gotSVGImages() {
            var thumbnailDataURL = IO.getThumbnail(
              data,
              width,
              height,
              120,
    ...
    function refreshLobby() {
      if (gn("hometab") !== null) {
        ...
        if (saveActual == saveExpected) {
          ...
          // Waiting for assets to be saved
