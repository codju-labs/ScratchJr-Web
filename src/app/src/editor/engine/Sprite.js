...
export default class Sprite {
  ...
  createSprite(page, md5, id, attr, fcn) {
    ...
    setProps(this, attr);
    if (
      Localization.isSampleLocalizedKey(this.name) &&
      ScratchJr.isSampleOrStarter()
    ) {
  ...
  setPos(dx, dy) {
    this.dirx =
      dx - this.xcoor == 0 ? 1 : (dx - this.xcoor) / Math.abs(dx - this.xcoor);
    this.diry =
      dy - this.ycoor == 0 ? 1 : (dy - this.ycoor) / Math.abs(dy - this.ycoor);
    this.xcoor = dx;
    this.ycoor = dy;
    this.wrap();
    this.render();
  ...
  openBalloon(label) {
    ...
    w = Math.round(w);
    var offset =
      this.screenLeft() + (this.div.offsetWidth * this.scale) / 2 - w / 2;
    ...
    this.balloon.style.webkitTransform =
  ...
  createText(attr, whenDone) {
    ...
    if (this.str == "" && !whenDone) {
      ...
      if (
        Localization.isSampleLocalizedKey(this.str) &&
        ScratchJr.isSampleOrStarter()
      ) {
  ...
  setTextBox() {
    ...
    if (isAndroid) {
      dy =
        box.y * scaleMultiplier + globaly(gn("stage")) - 10 * scaleMultiplier;
    } else {
    ...
    if (isAndroid) {
      ...
      dx =
        (-10 + 240 - Math.round(formsize / 2)) * scaleMultiplier +
        globalx(gn("stage"));
      ...
      dx =
        -10 +
        240 -
        Math.round(formsize / 2) +
        globalx(gn("stage"), gn("stage").offsetLeft);
      setProps(gn("textbox").style, {
        top: dy + "px",
        left: dx + "px",
        zIndex: 10,
  ...
  deleteText(record) {
    ...
    page.textstartat =
      this.ycoor + this.fontsize * 1.35 > 360 ? 36 : this.ycoor;
  ...
  startShaking() {
    ...
    var cb = newHTML(
      "div",
      this.type == "sprite" ? "deletesprite" : "deletetext",
      shake
    );
    ...
    if (globalx(cb) - globalx(ScratchJr.stage.div) < 0) {
      cb.style.left =
        Math.abs(globalx(cb) - globalx(ScratchJr.stage.div)) * this.scale +
        "px";
    }
    if (globaly(cb) - globaly(ScratchJr.stage.div) < 0) {
      cb.style.top =
        Math.abs(globaly(cb) - globaly(ScratchJr.stage.div)) * this.scale +
        "px";
    }
    cb.id = "deletesprite";
    this.div = shake;
  ...
  getData() {
    var data =
      this.type == "sprite" ? this.getSpriteData() : this.getTextBoxData();
    if (this.type != "sprite") {
      return data;
    }
