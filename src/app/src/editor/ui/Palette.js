...
export default class Palette {
  ...
  static createCategorySelectors(parent) {
    ...
    for (var i = 0; i < BlockSpecs.categories.length; i++) {
      Palette.createSelector(
        sel,
        i,
        leftPx + i * widthPx,
        0,
        BlockSpecs.categories[i]
      );
    }
  }
    ...
    for (var i = 0; i < pal.childElementCount; i++) {
      ...
      if (ScratchJr.shaking && ScratchJr.shaking == ths) {
        ...
        Events.startDrag(
          e,
          ths,
          Palette.prepareForDrag,
          Palette.dropBlockFromPalette,
          ScriptsPane.draggingBlock,
          Palette.showHelp,
          Palette.startShaking
        );
      }
    }
  ...
  static openPaletteBalloon(obj, label) {
    ...
    w =
      16 * window.devicePixelRatio * scaleMultiplier +
      getStringSize(
        ctx,
        "bold " + fontSize + "px " + window.Settings.paletteBalloonFont,
        label
      ).width;
    ...
    var dx =
      (globalx(obj) + obj.offsetWidth / 2) * window.devicePixelRatio - w / 2;
    ...
    writeText(
      ...
      "white",
      label,
      21 * window.devicePixelRatio * scaleMultiplier,
      8 * window.devicePixelRatio * scaleMultiplier
    );
  }
  ...
  static drawBalloon(ctx, w, h) {
    ...
    ctx.clearRect(
      0,
      0,
      Math.max(ctx.canvas.width, w),
      Math.max(ctx.canvas.height, h)
    );
  ...
  static prepareForDrag(e) {
    ...
    if (!Events.dragthumbnail.parentNode) {
      ...
      Events.dragthumbnail = Palette.getBlockNamed(
        Events.dragthumbnail.owner.blocktype
      );
    ...
    Events.dragcanvas = Events.dragthumbnail.owner.duplicateBlock(
      mx,
      my,
      sc.spr
    ).div;
  ...
  static selectCategory(n) {
    ...
    for (var i = 1; i < div.childElementCount; i++) {
      ...
      sel.childNodes[0].style.visibility =
        sel.index != n ? "visible" : "hidden";
      sel.childNodes[1].style.visibility =
        sel.index == n ? "visible" : "hidden";
    }
    var pal = gn("palette");
    ...
    for (var k = 0; k < list.length; k++) {
      if (list[k] == "space") {
        ...
        var newb = Palette.newScaledBlock(
          pal,
          list[k],
    ...
    dx += 30;
    if (
      n == BlockSpecs.categories.length - 1 &&
  ...
  static showSelectors(b) {
    ...
    for (var i = 0; i < div.childElementCount; i++) {
      var sel = div.childNodes[i];
      sel.childNodes[0].style.visibility =
        sel.index != n && b ? "visible" : "hidden";
      sel.childNodes[1].style.visibility =
        sel.index == n && b ? "visible" : "hidden";
      sel.childNodes[2].style.visibility =
        sel.index != n && b ? "visible" : "hidden";
      sel.childNodes[3].style.visibility =
        sel.index == n && b ? "visible" : "hidden";
    }
  }
  ...
  static drawRecordSound(w, h, dx) {
    ...
    var cnv = newCanvas(
      ...
      div.offsetHeight * window.devicePixelRatio,
      {
        webkitTransform:
          "translate(" +
          (-div.offsetWidth * window.devicePixelRatio) / 2 +
          "px, " +
          (-div.offsetHeight * window.devicePixelRatio) / 2 +
          "px) " +
          "scale(" +
          1 / window.devicePixelRatio +
          ") translate(" +
  ...
  static getLandingPlace(el, e, scale) {
    ...
    var box2 = new Rectangle(
      globalx(gn("palette")),
      globaly(gn("palette")),
      gn("palette").offsetWidth,
      gn("palette").offsetHeight
    );
    if (
      sc.flowCaret != null &&
      (sc.flowCaret.prev != null ||
        sc.flowCaret.next != null ||
        sc.flowCaret.inside != null)
    ) {
      return "scripts";
    }
    if (
      box2.overlapElemBy(box, 0.66) &&
      box2.hitRect({ x: el.left / scale, y: el.top / scale })
    ) {
      return "palette";
    }
    ...
    if (Palette.overlapsWith(gn("blockspalette"), box)) {
      /*bugfix - returning to blocks palette should delete block*/
      return "palette";
    }
  ...
  static getHittedThumb(el, div, scale) {
    ...
    for (var i = 0; i < div.childElementCount; i++) {
      ...
      var box2 = new Rectangle(
        globalx(node, node.offsetLeft),
        globaly(node, node.offsetTop) - dh,
        node.offsetWidth,
        node.offsetHeight
      );
