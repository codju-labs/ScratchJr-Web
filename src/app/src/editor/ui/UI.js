...
export default class UI {
  ...
  static finishTextFieldSave(dontHide) {
    ...
    var pname =
      ti.value.length == 0 ? ti.oldvalue : ti.value.substring(0, ti.maxLength);
  ...
  static enterFullScreen() {
    ...
    var scale = Math.min(
      (w - 136 * scaleMultiplier) / gn("stage").owner.width,
  ...
  static setMenuTextColor(t) {
    ...
    for (var i = 0; i < gn("textcolormenu").childElementCount; i++) {
      var mycolor =
        gn("textcolormenu").childNodes[i].childNodes[0].childNodes[0].style
          .backgroundColor;
      if (c == mycolor) {
