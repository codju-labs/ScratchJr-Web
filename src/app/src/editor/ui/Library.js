...
export default class Library {
  static init() {
    ...
    libFrame.style.minHeight =
      Math.max(getDocumentHeight(), frame.offsetHeight) + "px";
  ...
  static open(libType) {
    ...
    gn("okbut").onmousedown =
      type == "costumes"
        ? Library.closeSpriteSelection
        : Library.closeBkgSelection;
    Library.clean();
  ...
  static skipUserAssets() {
    ...
    Library.addHR(div);
    Library.displayLibAssets(
      type == "costumes" ? MediaLib.sprites : MediaLib.backgrounds
    );
  }
  ...
  static addLocalThumbChoose(parent, data, w, h, fcn) {
    ...
    img.style.left =
      Math.floor((w - scale * tb.w) / 2 + 9 * scaleMultiplier) + "px";
    img.style.top =
      Math.floor((h - scale * tb.h) / 2 + 9 * scaleMultiplier) + "px";
    img.style.position = "relative";
  ...
  static unSelect(tb) {
    ...
    if (clickThumb) {
      if (tb.byme && clickThumb.childElementCount > 1) {
        clickThumb.childNodes[
          clickThumb.childElementCount - 1
  ...
  static editCostume() {
    var sname;
    var cname = selectedOne
      ? clickThumb.fieldname
      : Localization.localize("LIBRARY_CHARACTER");
    var scale = selectedOne && selectedOne != "none" ? clickThumb.scale : 0.5;
    var md5 = selectedOne && selectedOne != "none" ? selectedOne : undefined;
    var w =
      selectedOne && selectedOne != "none"
        ? Math.round(clickThumb.w)
        : undefined;
    var h =
      selectedOne && selectedOne != "none"
        ? Math.round(clickThumb.h)
        : undefined;
  ...
  static closeBkgSelection(e) {
    ...
    e.stopPropagation();
    if (selectedOne) {
      ScratchJr.stage.currentPage.setBackground(
        selectedOne,
