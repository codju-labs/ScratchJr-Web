...
export default class Project {
  ...
  static startLoad() {
    ScratchJr.log(
      "all UI assets recieved - procced to call server",
      ScratchJr.getTime(),
      "sec"
    );
  ...
  static getStarted(whenDone) {
    ...
    interval = null;
    ScratchJr.log(
      "Project images retrieved from server",
      ScratchJr.getTime(),
      "sec"
    );
  ...
  static recreate(data) {
    ScratchJr.log(
      "Project data structures start loading",
      ScratchJr.getTime(),
      "sec"
    );
    loading = true;
  ...
  static substractCount() {
    mediaCount--;
    if (
      gn("backdrop").className != "modal-backdrop fade in" ||
      mediaCountBase == 0
    ) {
      return;
    }
  ...
  static encodeStrip(b) {
    ...
    while (b != null) {
      ...
      var arg =
        b.arg != null || hasargs.indexOf(bt) > -1 ? b.getArgValue() : null;
      if (!arg && arg != 0) {
        arg = "null";
      }
