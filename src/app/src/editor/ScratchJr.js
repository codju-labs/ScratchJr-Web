...
export default class ScratchJr {
  ...
  static appinit(v) {
    console.log("1 - Running App Init", v);
  ...
  static onNumberKeyDown(e) {
    if (
      ScratchJr.isNumberPadKeyCode(e) &&
      document.getElementsByClassName("picokeyboard on").length > 0
    ) {
      e.preventDefault();
      e.stopPropagation();
  ...
  static goBack() {
    if (onBackButtonCallback.length === 0) {
      ...
      var callbackReference =
        onBackButtonCallback[onBackButtonCallback.length - 1];
      callbackReference();
    }
  }
}
