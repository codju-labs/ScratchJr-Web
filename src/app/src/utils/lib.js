...
export function globalx(el) {
  ...
  while (el && el.offsetLeft != undefined) {
    var webkitTransform = new WebKitCSSMatrix(
...
export function localy(el, gy) {
  ...
  while (el && el.offsetTop != undefined) {
    ly -=
      el.offsetTop +
      el.clientTop +
      new WebKitCSSMatrix(window.getComputedStyle(el).webkitTransform).m42;
    el = el.parentNode;
  }
  return ly;
}
  ...
  for (var i = 0; i < ss.length; ++i) {
    for (var j = 0; j < ss[i].cssRules.length; ++j) {
      ...
      for (var k = 0; k < styles.length; ++k) {
        if (
          styles[k].type == window.CSSRule.WEBKIT_KEYFRAMES_RULE &&
          styles[k].name == rule
        ) {
          return styles[k];
        }
