...
const DEBUG = true; // grab the DEBUG variable from main. This is passed through the BrowserWindow creation
...
class ElectronDesktopInterface {
  ...
  io_registersound(dir, name) {
    if (!this.currentAudio[name]) {
      ...
      this.loadSoundFromDataURI(name, `sounds/${name}`);
    }
  }
  ...
  io_setfile(name, btoa_str) {
    ...
    return ipcRenderer.sendSync("io_setfile", {
      name: name,
      contents: btoa_str,
    });
  }
      ...
      if (keep === "YES") {
        ...
        if (blob) {
          ...
          fileReader.onload = function () {
            ...
            electronDesktopInterface.io_setmedianame(
              fileReader.result,
              filename,
              "webm"
            );
            electronDesktopInterface.loadSoundFromDataURI(
              filename + ".webm",
              fileReader.result
            );
          };
...
(function () {
  ...
  let promisifiedOldGUM = function (
    constraints,
    successCallback,
    errorCallback
  ) {
...
class AudioCapture {
  ...
  startAudioMeter(clipLevel, averaging, clipLag) {
    ...
    if (!this.audioProcessor) {
      ...
      let processor = audioContext.createScriptProcessor(
        typeof AudioContext != "undefined" ? null : 512,
        1,
        1
      );
  ...
  processVolume(event) {
    ...
    if (DEBUG_AUDIOMETER)
      debugLog("process volume:", buf, sum, avg, this.audioProcessor);
  }
} // AudioCapture
