...
const DEBUG = isDev;
const DEBUG_DATABASE = DEBUG && true;
...
ipcMain.on("io_getIsDebug", (event, arg) => {
...
ipcMain.on("io_setfile", (event, arg) => {
  ...
  event.returnValue = dataStore.writeProjectFile(arg.name, arg.contents, {
    encoding: "utf8",
  });
});
...
ipcMain.on("io_getAudioData", (event, audioName) => {
  ...
  if (!filePath) {
    ...
    filePath = dataStore.safeGetFilenameInAppDirectory(
      `sounds/${audioName}`,
      false
    );
  }
  ...
  if (!filePath) {
    ...
    if (DEBUG_FILEIO)
      debugLog("...trying to look in the PROJECTFILE table", audioName);

    // this is already stored as a string, we do not need to convert it
    let projectDBFile =
      dataStore.readProjectFileAsBase64EncodedString(audioName);
    if (DEBUG_FILEIO && !projectDBFile)
      debugLog("...WARNING: unable to find: ", audioName);
  ...
  if (!data) {
    if (DEBUG_FILEIO)
      debugLog("io_getAudioData - could not find on disk", audioName, filePath);
...
class ScratchJRDataStore {
  ...
  restoreProjects() {
    ...
    if (fs.existsSync(scratchRestoreDB)) {
      ...
      if (DEBUG_DATABASE)
        debugLog("DatabaseManager reloaded from restored copy");
      ...
      dialog.showErrorBox(
        "Database Restored",
        "The database not been restored.  Could not find file: " +
          scratchRestoreDB
      );
    }
  }
  ...
  safeGetFilenameInAppDirectory(file, warnIfNotPresent) {
    ...
    if (DEBUG_FILEIO || warnIfNotPresent)
      debugLog(
        "safeGetFilenameInAppDirectory: file does not exist.",
        file,
        filePath
      );
...
class DatabaseManager {
  constructor(databaseFilename, databaseRestoreFilename) {
    console.log("d: Running Constructor");
    if (DEBUG_DATABASE) debugLog("DatabaseManager created");
  ...
  open() {
    const fileToOpen = this.databaseRestoreFilename
      ? this.databaseRestoreFilename
      : this.databaseFilename;

    console.log("WE WANT TO OPEN", fileToOpen);
  ...
  cleanProjectFiles(fileType) {
    ...
    for (let i = 0; i < allProjectFilesWithExtension.length; i++) {
      ...
      if (DEBUG_CLEANASSETS)
        debugLog("checking if in use: ", currentFileToCheck);
      ...
      if (projectJSON.length > 0) {
        if (DEBUG_CLEANASSETS)
          debugLog("...project is currently using: ", currentFileToCheck);
      ...
      if (shapeFiles.length > 0) {
        if (DEBUG_CLEANASSETS)
          debugLog("...user shapes is using: ", currentFileToCheck, shapeFiles);
        continue; // we don't need to keep checking this file, it is being used
      }
      ...
      if (DEBUG_CLEANASSETS)
        debugLog("...not in use, removing: ", currentFileToCheck);
  ...
  stmt(jsonStrOrJsonObj) {
    try {
      ...
      if (DEBUG_DATABASE)
        debugLog("DatabaseManager executing stmt", stmt, values);
...
var logFile = fs.createWriteStream(
  path.join(ScratchJRDataStore.getScratchJRFolder(), "debug.log"),
  { flags: "a" }
);
